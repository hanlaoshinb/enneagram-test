...
window.handleAnswer = function(answer) {
    console.log("全局处理答案函数被调用:", answer);
    // 此函数将在DOM加载后被重新定义
};

document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM 加载完成，初始化测试...");
    ...
    function showResult() {
        ...
        try {
            ...
            console.log("计算出的类型:", personalityType);
            
            const result = personalityTypes[personalityType];
            console.log("人格特征:", result);
            
            if (!result) {
                throw new Error(`未找到类型 ${personalityType} 的信息`);
            }
            ...
            testContainer.innerHTML = `
                <div class="min-h-screen flex flex-col items-center justify-center p-6">
                    <div class="w-full max-w-3xl bg-white rounded-2xl p-8 shadow-lg">
                        <h2 class="text-3xl font-bold mb-6 text-center text-red-600">出错了</h2>
                        <p class="text-gray-600 mb-8">很抱歉，生成结果时出现了问题。请刷新页面重试。</p>
    ...
    document.querySelectorAll('button').forEach(button => {
        ...
        if (text === '开始测试' || text === '立即开始测试') {
            console.log("找到测试按钮:", text);
